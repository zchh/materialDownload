...
class Login extends Base
{
   ...
   public function adminLogin()
   {
       if($_SERVER['REQUEST_METHOD'] == "POST"){
           ...
           if(password_verify($requestParam['password'], $selectResult[0]['password'])){
               setcookie('admin_id', $selectResult[0]['admin_id'],time()+24*60*60*7,'/');
               //将用户信息的密码加盐加密保存在cookie中
               setcookie('admin_password',password_hash($selectResult[0]['password'],PASSWORD_DEFAULT),time()+24*60*60*7,'/');
